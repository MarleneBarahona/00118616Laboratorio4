void createLocalA(Matrix &A,mesh m){
    float tau = m.getParameter(CONSTANT_TAU);
...
void createLocalD(Matrix &D,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
	float v = m.getParameter(CONSTANT_V);
    D.at(0).at(0) += v/l;  D.at(0).at(1) += -v/l;
    D.at(1).at(0) += -v/l;  D.at(1).at(1) += v/l;
}

void createLocalP(Matrix &P, mesh m){
	float alpha = m.getParameter(CONSTANT_ALPHA);
	P.at(0).at(0) += -(3*alpha)/2;  P.at(0).at(1) += (3*alpha)/2;
    P.at(1).at(0) += -(3*alpha)/2;  P.at(1).at(1) += (3*alpha)/2;
}

void createLocalR(Matrix &R, mesh m){
	float delta = m.getParameter(CONSTANT_DELTA);
	R.at(0).at(0) += -(delta)/2;  R.at(0).at(1) += (delta)/2;
    R.at(1).at(0) += -(delta)/2;  R.at(1).at(1) += (delta)/2;
}
Matrix createLocalK(int element,mesh &m){
    Matrix K,A,B,C,D,P,R;
    ...
    row3.push_back(P.at(0).at(0)); 
    row3.push_back(P.at(0).at(1));
    row3.push_back(R.at(0).at(0)); 
    row3.push_back(R.at(0).at(1));

    row4.push_back(P.at(1).at(0)); 
    row4.push_back(P.at(1).at(1));
    row4.push_back(R.at(1).at(0)); 
    row4.push_back(R.at(1).at(1));
...
Vector createLocalb(int element,mesh &m){
    ...
    float f = m.getParameter(CONSTANT_V), l = m.getParameter(ELEMENT_LENGTH);
